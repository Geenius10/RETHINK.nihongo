...
function readingFor(c){
  // Romaji mode must never show kanji/kana. Prefer the audited Latin field first.
  const hasJapanese=x=>/[\u3040-\u30ff\u3400-\u9fff]/.test(String(x||''));
  const cleanLatin=x=>String(x||'').trim().replace(/^[·・\s]+|[·・\s]+$/g,'');
  const stored=cleanLatin(c&&c.romaji);
  if(stored && /[A-Za-zāīūēōĀĪŪĒŌ]/.test(stored) && !hasJapanese(stored)) return stored;
  const r=String(c&&c.reading||'').trim();
  if(r){
    const parts=r.split(/[·・]/).map(cleanLatin).filter(Boolean);
    const latin=parts.slice().reverse().find(x=>/[A-Za-zāīūēōĀĪŪĒŌ]/.test(x)&&!hasJapanese(x));
    if(latin)return latin;
    // A kana reading is converted to Latin characters here.
    if(/^[ぁ-んァ-ヶー\s]+$/.test(r)){
      const converted=romajiJP(r);
      if(converted&&!hasJapanese(converted))return converted;
    }
  }
  const front=String(c&&c.front||'').trim();
  if(/^[ぁ-んァ-ヶー\s]+$/.test(front)){
    const converted=romajiJP(front);
    if(converted&&!hasJapanese(converted))return converted;
  }
  // Never leak Japanese script into the field labelled Romaji.
  return '';
}
...
function boot(){
  ...
  window.__RN_BOOTED=true;try{const badge=$('buildBadge');if(badge)badge.textContent='Build 19 · fast engine'}catch(e){}
